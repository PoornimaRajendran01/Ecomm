...
const config= configuration.find(config=>config.id === SELECTED_CONFIGURATION);
useEffect(()=>{
if(config){
    localStorage.setItem('config',JSON.stringify(config));
    document.getElementById('root').style.background= config.theme.backgroundcolor
}
},[config]);
            ...
            >
              <Switch>
                ...
                <Route
                  ...
                  component={config.theme.home === 1 ? HomeFashion : HomeFashionTwo}
                />
                <Route
